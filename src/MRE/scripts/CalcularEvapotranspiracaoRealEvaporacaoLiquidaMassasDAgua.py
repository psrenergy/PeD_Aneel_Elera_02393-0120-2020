...
def CalcularEvaporacaoLiquidaHistoricaMassasDAgua(shapefileMassaDAguaLandsatAgregado, anoInicialHistorico, anoFinalHistorico):    
    """
    Calcula a evaporação líquida  histórica de cada massa dagua definida no shapefileMassaDAguaLandsatAgregado, para o período informado.
    
    Evaporação líquida massa d'água = Evaporação da superfície da massa d'água - Evapotranspiração real
        Evaporação da superfície da massa d'água = Kw * Evaporação de referência
        onde: 
            Kw =0,3814 NDWI + 0,5989
            onde: 
                NDWI = índice de água por diferença normalizada
        
        Evapotranspiração real = evapotranspiração real que ocorreria na mesma área, se não existisse o lago.
    ...
    Observacao: por hora, nao esta sendo usado o fator Kw para o computo da  Evaporação da superfície da massa d'água. 
    Logo, Evaporação da superfície da massa d'água  = Evaporação de referência 
    
    @param shapefileMassaDAguaLandsatAgregado: feature class dos pontos das margens das massas dagua
...
def calcularMediaHistoricaEvaporacaoReferenciaPorMassaDagua(camadaPontosMargemMassasDAgua, anoInicialHistorico, anoFinalHistorico):
    """
    Calcula a evaporação de referência média de cada massa dagua, para o período histórico informado.
    Processo:
        Para cada massa dagua da camada camadaPontosMargemMassasDAgua, extrai-se da superfície interpolada de evaporação de referência, 
        os valores de evaporação em cada ponto ou vertice da massa dagua. Computa-se a media de evaporacao da massa dagua, a patir dos valores
        de referencia em seus vertices. 
        ...
        for ano in range(anoInicialHistorico, anoFinalHistorico + 1):
            ...
            for mes in range(1,13):
                ...
                if arcpy.Exists(nomeRasterEvapRefMediaMunicipio):
                    ...
                    #Filtra a camada de pontos (vertices das massas dagua) pelo ano e estacao corrente
...
def CalcularEvapotranspiracaoRealHistoricaMassasDAgua (shapefileMassaDAguaLandsatAgregado, shapefileMassaDAguaANA):
    ...
    Metodologia: para cada massa d'água:
       ...
       3. Extrai a evapotranspiração real de todos os pontos de todas as massas d'água, a partir do raster do produto SSeBop Global
    ...
    try:
        ...
        for ano in range(anoInicialHistorico, anoFinalHistorico + 1):
            ...
            for mes in range(1,13):
                ...
                #Monta nome do raster de evapotranspiração real (SSEBop) referente ao período <ano,mes>
...
CalcularEvaporacaoLiquidaHistoricaMassasDAgua(shapefileMassaDAguaLandsatAgregado, anoInicialHistorico, anoFinalHistorico)    
