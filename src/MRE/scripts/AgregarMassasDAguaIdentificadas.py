...
camadaMassaDAguaLandsatAgregado = arcpy.GetParameterAsText(1)

if not arcpy.Exists(diretorioShapeFilesMassaDAguaLandSat):
    arcpy.AddError("Diretorio inexistente: {dir}".format(dir=diretorioShapeFilesMassaDAguaLandSat) )
    sys.exit(0)

nomeCamadaAgregada = os.path.basename(camadaMassaDAguaLandsatAgregado)
workspace =os.path.dirname(camadaMassaDAguaLandsatAgregado)

arcpy.AddMessage(time.ctime() + " - Agregando as Massas DAgua de cada imagem Landsat em uma unica camada: {file}...".format(file = os.path.basename(camadaMassaDAguaLandsatAgregado)))
...
for nomeArquivoLandsat in os.listdir(diretorioShapeFilesMassaDAguaLandSat):
    if nomeArquivoLandsat.endswith("dissolve.shp"):
        ...
        if cont == 1:
            ...
            arquivoShapeFileMassasDAguaAgregado= configurarShapeFilesMassaDAguaLandSatAgregado(workspace, nomeCamadaAgregada)
