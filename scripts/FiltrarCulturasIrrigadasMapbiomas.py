...
def filtrarCulturasIrrigadas(anoInicialHistorico, anoFinalHistorico):
    """
    Filtra das camadas do Mapbiomas as culturas que tem inteserseção com as áreras identificadas
    como irrigadas no conjunto de imagens Landsat
    """
    pymsg=''
    msgs =''
    ...
    try:
        ...
        arcpy.AddMessage('Filtrando os cultivos do mapemanto do Mapbiomas, que tem tem intersecao com as areas irrigadas mapeadas no LandSat...')
        ...
        for ano in range(anoInicialHistorico, anoFinalHistorico + 1):
            ...
            nomeRasterMapBiomasFiltrado = "mapbiomas_brazil_collection_60_" + str(ano) + "_cultivos_irrigados" 

            if arcpy.Exists(nomeRasterMapBiomas) and arcpy.Exists(nomeRasterLandSatIrrigacao):

                arcpy.AddMessage('Filtrando as cultivos irrigados do arquivo Mapbiomas: {Mapbiomas}'.format(Mapbiomas = os.path.basename(nomeRasterLandSatIrrigacao)))
        
                # Processo: Extract by Mask
                rasterEntrada = nomeRasterMapBiomas
                rasterMascara = nomeRasterLandSatIrrigacao
                rasterResultado = nomeRasterMapBiomasFiltrado
                
                nomeRasterMapBiomasFiltrado = arcpy.sa.ExtractByMask(rasterEntrada, rasterMascara)
                nomeRasterMapBiomasFiltrado.save(rasterResultado)
...
if not arcpy.Exists(diretorioLandSatAreasIrrigadas):
